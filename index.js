...
bot.on('message', message => {
        ...
        async function fetchgitdata() {
            try {
                ...
                for (let modlogs = 0; modlogs < modnums; modlogs++) {
                    if (gitdata[modlogs].type === 'dir') {
                        gitdata[modlogs].type = 'folder'
                    }
                    filenames.push((gitdata[modlogs].name) + '- Type: ' + (gitdata[modlogs].type))
                }
                filenames.join("\n")
                let filegitstr = filenames.join("\n")
                let fileembed = new Discord.MessageEmbed().setThumbnail('https://cdn.discordapp.com/attachments/719578195371884565/719588675851124816/og-image-removebg-preview.png').setTitle("<:githubsuccess:719971373317095547> Here are the Github files from " + datalink + '/' + args[1] + '.').setColor('#0099ff').setDescription('```' + filegitstr + '```')
                message.channel.send(fileembed)
            } catch {
                const errmessage = new Discord.MessageEmbed().setColor('#0099ff').setTitle("<:githuberror:719971585716387862> There was an internal error, possibly with the link. Make sure the file/folder you entered is valid. (Make sure to only run this command on folders.)")
    ...
    case ';files':
        async function fetchrawdata() {
            try {
                ...
                let fileembed = new Discord.MessageEmbed().setThumbnail('https://cdn.discordapp.com/attachments/719578195371884565/719588675851124816/og-image-removebg-preview.png').setTitle("<:githubsuccess:719971373317095547> Here are the raw file links from " + datalink + '/' + args[1] + '.').setColor('#0099ff').setDescription('**' + gitstr + '**').addField('Note:', 'The order of the files is respective to the order that they appear in `;github`.')
                message.channel.send(fileembed)
            } catch {
                const errmessage = new Discord.MessageEmbed().setColor('#0099ff').setTitle("<:githuberror:719971585716387862> There was an internal error, possibly with the link. Reverting link to default. Make sure the file/folder you entered is valid. (Make sure to only run this command on folders.)")
                message.channel.send(errmessage)
                datalink = "https://api.github.com/repos/Prodigy-Hacking/ProdigyMathGameHacking/contents"
                rawcontentlink = ""
            }
    ...
    case ';ping':
        ...
        const pingresponse = new Discord.MessageEmbed().setColor('#0099ff').setTitle('Pong! Latency is ' + '`' + ping + '`')
    ...
    case ';repoinfo':
        async function fetchGenData() {
            ...
            const gendataembed = new Discord.MessageEmbed().setColor('#0099ff').setTitle('ProdigyMathGameHacking repository - General data.').addField('Repository name:', gendata.name, false).addField('Description:', gendata.description, false).addField('Number of watches:', gendata.watchers, false).addField('Number of forks:', gendata.forks, false).addField('Api URL:', gendata.url, false).addField('Github URL:', gendata.svn_url, false).addField('Repo ID:', gendata.id, false).addField('Website:', gendata.homepage, false).addField('Current repository issues:', gendata.open_issues, false).addField('Owner/Organization:', gendata.organization.login, false).addField('License:', gendata.license.name, false)
    ...
    case ';contributors':
        async function fetchContData() {
            ...
            const contembed = new Discord.MessageEmbed().setColor('#0099ff').setTitle('<:githubsuccess:719971373317095547> Here are the current Github contributors:').setDescription('```' + contgitstr + '```')
    ...
    case ';issues':
        async function fetchIssueData() {
            ...
            const issueEmbed = new Discord.MessageEmbed().setColor('#0099ff').setTitle('<:githubsuccess:719971373317095547> Here are the current open issues.').setDescription('**' + finalIssueData + '**')
    ...
    case ';tags':
        async function fetchTagData() {
            ...
            const tagembed = new Discord.MessageEmbed().setColor('#0099ff').setTitle('<:githubsuccess:719971373317095547> Here are the current issue tags.').setDescription(finalTagData)
    ...
    case ';traffic':
        async function fetchTrafficData() {
            try {
                ...
                }).then(response => response.json()).then(data => viewdata = data);
                setTimeout(function () {
                    const trafficEmbed = new Discord.MessageEmbed().setColor('#0099ff').setTitle('<:githubsuccess:719971373317095547> Here is the current Github traffic.').setDescription('Here is the traffic for this week: `' + viewdata.count + '`. Unique viewers: `' + viewdata.uniques + '`.\n Here is the traffic for today: Total views: `' + viewdata.views[viewdata.views.length - 1].count + '`. Unique viewers: `' + viewdata.views[viewdata.views.length - 1].uniques + '`.')
                    message.channel.send(trafficEmbed)
                }, 1000)
            } catch {
                const trafficErr = new Discord.MessageEmbed().setColor('#0099ff').setTitle('<:githuberror:719971585716387862> I was not able to fetch the traffic data.')
